...
int (read_KBC_output)(uint8_t port, uint8_t *output, int mouse){
    ...
    {
        /* code */
        if (read_KBC_status(&status) != 0 || status & TIMEOUT_ERROR || status & PARITY_ERROR){
            printf("Error: Error reading KBC status!\n");
            return 1; //lê o registo de status
        }
        if(status & FULL_OUT_BUFFER){
            if ((mouse && (status & MOUSE_TICK)) || (!mouse && !(status & MOUSE_TICK))){
        ...
        tickdelay(micros_to_ticks(WAIT_KBC)); //delay de 20ms
        k++;
    }
