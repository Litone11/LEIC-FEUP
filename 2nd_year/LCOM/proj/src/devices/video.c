...
static uint8_t *video_mem, *video_mem2;

int(buf_swap)(){
  memcpy(video_mem, video_mem2, mode_info.XResolution * mode_info.YResolution * n_bytes_per_pixel);
  return 0;
}
...
int (vg_draw_pixel)(uint16_t x, uint16_t y, uint32_t color) {
  ...
  if (color == 0x00000000) return 0; // If color is black, do not draw anything
  unsigned int index = (mode_info.XResolution * y + x) * n_bytes_per_pixel;
  if (memcpy(&video_mem2[index], &color, n_bytes_per_pixel) == NULL) return 0;
  return 0;
}
