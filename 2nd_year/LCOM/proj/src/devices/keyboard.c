...
int (keyboard_subscribe_interrupts)(uint8_t *bit_no) {
    ...
    *bit_no = BIT(hook_id_keyboard);
    return sys_irqsetpolicy(IRQ_KBC, IRQ_REENABLE | IRQ_EXCLUSIVE, &hook_id_keyboard);
}
...
int (enableInterrupt)(){
    uint8_t status;
    if (write_KBC_command(KBC_IN_CMD, KBC_READ_CMD, 0) != 0) return 1;
    if (read_KBC_output(KBC_OUT_CMD, &status, 0) != 0) return 1;
    if (write_KBC_command(KBC_IN_CMD, KBC_WRITE_CMD, 0) != 0) return 1;
    return (write_KBC_command(KBC_IN_CMD, status | BIT(0), 0) != 0);
}
