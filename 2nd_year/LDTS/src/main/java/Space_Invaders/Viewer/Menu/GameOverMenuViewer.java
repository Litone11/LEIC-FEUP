...
public class GameOverMenuViewer extends MenuViewer<GameOverMenu> {
    ...
    public GameOverMenuViewer(GameOverMenu menu) {super(menu, new Position(3,14));}


    @Override
    protected void drawElements(GUI gui, long time) {
        drawOptions(gui);
        gui.drawText(new Position(27,11),"score ", Theme.getTheme().menuColorSelected );
        gui.drawText(new Position(28,13),"" + getModel().getScore(), Theme.getTheme().menuColor );


        super.drawMenuTitle(gui, "OVER", Theme.getTheme().menuColorTitle2, new Position(29, 5));
        gui.drawText(new Position(25,3), "GAME", Theme.getTheme().menuColorTitle1);
